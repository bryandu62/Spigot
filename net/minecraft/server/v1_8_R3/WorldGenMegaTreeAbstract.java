  ...
  extends WorldGenTreeAbstract
{
      ...
      for (int j = 0; j <= 1 + i; j++)
      {
        ...
        for (int k = -b0; (k <= b0) && (flag); k++) {
          for (int l = -b0; (l <= b0) && (flag); l++) {
            if ((blockposition.getY() + j < 0) || (blockposition.getY() + j >= 256) || ((!a(world.getType(blockposition.a(k, j, l)).getBlock())) && (world.getType(blockposition.a(k, j, l)).getBlock() != Blocks.SAPLING))) {
              flag = false;
            }
          }
        }
  ...
  {
    return (c(world, blockposition, i)) && (a(blockposition, world));
  }
