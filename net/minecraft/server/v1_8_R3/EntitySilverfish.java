  ...
  extends EntityMonster
{
  ...
  public EntitySilverfish(World world)
  {
    ...
    this.goalSelector.a(3, this.a = new PathfinderGoalSilverfishWakeOthers(this));
    ...
    extends PathfinderGoalRandomStroll
  {
    ...
    public boolean a()
    {
      ...
      if (!this.silverfish.getNavigation().m()) {
        return false;
      }
          ...
          for (int j = 0; (j <= 10) && (j >= -10); j = j <= 0 ? 1 - j : 0 - j) {
            for (int k = 0; (k <= 10) && (k >= -10); k = k <= 0 ? 1 - k : 0 - k)
            {
              ...
              if (iblockdata.getBlock() == Blocks.MONSTER_EGG) {
                if (!CraftEventFactory.callEntityChangeBlockEvent(this.silverfish, blockposition1.getX(), blockposition1.getY(), blockposition1.getZ(), Blocks.AIR, 0).isCancelled())
                {
