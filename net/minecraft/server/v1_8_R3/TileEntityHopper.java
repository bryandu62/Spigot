...
public class TileEntityHopper
  ...
  implements IHopper, IUpdatePlayerListBox
{
  ...
  public boolean m()
  {
    if ((this.world != null) && (!this.world.isClientSide))
    {
      if ((!n()) && (BlockHopper.f(u())))
      {
        ...
        if (flag)
        {
          d(this.world.spigotConfig.hopperTransfer);
          update();
          return true;
        }
      }
  ...
  private boolean r()
  {
    IInventory iinventory = H();
    if (iinventory == null) {
      return false;
    }
    EnumDirection enumdirection = BlockHopper.b(u()).opposite();
    if (a(iinventory, enumdirection)) {
      return false;
    }
    for (int i = 0; i < getSize(); i++) {
      if (getItem(i) != null)
      {
        ...
        CraftItemStack oitemstack = CraftItemStack.asCraftMirror(splitStack(i, this.world.spigotConfig.hopperAmount));
  ...
  public static ItemStack addItem(IInventory iinventory, ItemStack itemstack, EnumDirection enumdirection)
  {
    if (((iinventory instanceof IWorldInventory)) && (enumdirection != null))
    {
      ...
      do
      {
        itemstack = c(iinventory, itemstack, aint[i], enumdirection);i++;
        if ((i >= aint.length) || (itemstack == null)) {
          break;
        }
  ...
  private IInventory H()
  {
    ...
    return b(getWorld(), this.position.getX() + enumdirection.getAdjacentX(), this.position.getY() + enumdirection.getAdjacentY(), this.position.getZ() + enumdirection.getAdjacentZ());
  }
  ...
  public static IInventory b(World world, double d0, double d1, double d2)
  {
    ...
    if (object == null)
    {
      List list = world.a(null, new AxisAlignedBB(d0 - 0.5D, d1 - 0.5D, d2 - 0.5D, d0 + 0.5D, d1 + 0.5D, d2 + 0.5D), IEntitySelector.c);
      if (list.size() > 0) {
