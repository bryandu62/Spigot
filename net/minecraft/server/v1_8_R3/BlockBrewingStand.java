  ...
  extends BlockContainer
{
  ...
  {
    super(Material.ORE);
    j(this.blockStateList.getBlockData().set(HAS_BOTTLE[0], Boolean.valueOf(false)).set(HAS_BOTTLE[1], Boolean.valueOf(false)).set(HAS_BOTTLE[2], Boolean.valueOf(false)));
  }
  ...
  public IBlockData fromLegacyData(int i)
  {
    ...
    for (int j = 0; j < 3; j++) {
      iblockdata = iblockdata.set(HAS_BOTTLE[j], Boolean.valueOf((i & 1 << j) > 0));
    }
    return iblockdata;
  }
  ...
  public int toLegacyData(IBlockData iblockdata)
  {
    int i = 0;
    for (int j = 0; j < 3; j++) {
      if (((Boolean)iblockdata.get(HAS_BOTTLE[j])).booleanValue()) {
        i |= 1 << j;
      }
    }
    return i;
