  ...
  extends CommandAbstract
{
    ...
    throws CommandException
  {
    ...
    ICommandListener icommandlistener1 = new ICommandListener()
    {
      ...
      public Vec3D d()
      {
        return new Vec3D(d0, d2, this.val$d2);
      }
  ...
  {
    return (astring.length == 9) && ("detect".equals(astring[4])) ? a(astring, Block.REGISTRY.keySet()) : (astring.length > 5) && (astring.length <= 8) && ("detect".equals(astring[4])) ? a(astring, 5, blockposition) : (astring.length > 1) && (astring.length <= 4) ? a(astring, 1, blockposition) : astring.length == 1 ? a(astring, MinecraftServer.getServer().getPlayers()) : null;
  }
