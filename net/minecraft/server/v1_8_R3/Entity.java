  ...
  implements ICommandListener
{
  ...
  {
    AxisAlignedBB axisalignedbb = getBoundingBox().c(d0, d1, d2);
    ...
    else
    {
      ...
      if (flag)
      {
        ...
        for (;;)
        {
          ...
          d6 = d0;
          if (d0 != 0.0D) {
            if (!this.world.getCubes(this, getBoundingBox().c(d0, -1.0D, 0.0D)).isEmpty()) {
              break;
            }
          }
        }
      ...
      for (Iterator iterator1 = list.iterator(); iterator1.hasNext(); d0 = axisalignedbb2.a(getBoundingBox(), d0)) {
  ...
  protected void checkBlockCollisions()
  {
    ...
    BlockPosition blockposition1 = new BlockPosition(getBoundingBox().d - 0.001D, getBoundingBox().e - 0.001D, getBoundingBox().f - 0.001D);
  ...
  protected void a(BlockPosition blockposition, Block block)
  {
    ...
    if (this.world.getType(blockposition.up()).getBlock() == Blocks.SNOW_LAYER)
    {
      ...
      makeSound(block_stepsound.getStepSound(), block_stepsound.getVolume1() * 0.15F, block_stepsound.getVolume2());
    }
  ...
  public boolean U()
  {
    return (this.inWater) || (this.world.isRainingAt(new BlockPosition(this.locX, this.locY, this.locZ))) || (this.world.isRainingAt(new BlockPosition(this.locX, this.locY + this.length, this.locZ)));
  }
  ...
  protected void X()
  {
    ...
    makeSound(aa(), f, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.4F);
    float f1 = MathHelper.floor(getBoundingBox().b);
    for (int i = 0; i < 1.0F + this.width * 20.0F; i++)
    {
      ...
      this.world.addParticle(EnumParticle.WATER_BUBBLE, this.locX + f2, f1 + 1.0F, this.locZ + f3, this.motX, this.motY - this.random.nextFloat() * 0.2F, this.motZ, new int[0]);
    }
    for (i = 0; i < 1.0F + this.width * 20.0F; i++)
    {
      float f2 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
      float f3 = (this.random.nextFloat() * 2.0F - 1.0F) * this.width;
      this.world.addParticle(EnumParticle.WATER_SPLASH, this.locX + f2, f1 + 1.0F, this.locZ + f3, this.motX, this.motY, this.motZ, new int[0]);
    }
  }
  ...
  protected void Z()
  {
    ...
    if (block.b() != -1) {
      this.world.addParticle(EnumParticle.BLOCK_CRACK, this.locX + (this.random.nextFloat() - 0.5D) * this.width, getBoundingBox().b + 0.1D, this.locZ + (this.random.nextFloat() - 0.5D) * this.width, -this.motX * 4.0D, 1.5D, -this.motZ * 4.0D, new int[] { Block.getCombinedId(iblockdata) });
    }
  }
  ...
  public void e(NBTTagCompound nbttagcompound)
  {
    try
    {
      nbttagcompound.set("Pos", a(new double[] { this.locX, this.locY, this.locZ }));
  ...
  public void f(NBTTagCompound nbttagcompound)
  {
    try
    {
      ...
      b(nbttagcompound.getBoolean("Silent"));
      a(nbttagcompound);
      if (af()) {
        setPosition(this.locX, this.locY, this.locZ);
      }
  ...
  public boolean inBlock()
  {
    ...
    for (int i = 0; i < 8; i++)
    {
      int j = MathHelper.floor(this.locY + ((i >> 0) % 2 - 0.5F) * 0.1F + getHeadHeight());
  ...
  public void mount(Entity entity)
  {
    ...
    if (entity == null)
    {
      if (this.vehicle != null)
      {
        ...
        pluginManager.callEvent(new EntityDismountEvent(getBukkitEntity(), this.vehicle.getBukkitEntity()));
    ...
    else
    {
      ...
      {
        EntityMountEvent event = new EntityMountEvent(getBukkitEntity(), entity.getBukkitEntity());
        pluginManager.callEvent(event);
        if (event.isCancelled()) {
          return;
        }
  ...
  public void d(BlockPosition blockposition)
  {
    ...
    else
    {
      if ((!this.world.isClientSide) && (!blockposition.equals(this.an)))
      {
        ...
        double d0 = shapedetector_shapedetectorcollection.b().k() == EnumDirection.EnumAxis.X ? shapedetector_shapedetectorcollection.a().getZ() : shapedetector_shapedetectorcollection.a().getX();
        double d1 = shapedetector_shapedetectorcollection.b().k() == EnumDirection.EnumAxis.X ? this.locZ : this.locX;
        
        d1 = Math.abs(MathHelper.c(d1 - (shapedetector_shapedetectorcollection.b().e().c() == EnumDirection.EnumAxisDirection.NEGATIVE ? 1 : 0), d0, d0 - shapedetector_shapedetectorcollection.d()));
  ...
  public void onLightningStrike(EntityLightning entitylightning)
  {
    ...
    org.bukkit.craftbukkit.v1_8_R3.event.CraftEventFactory.entityDamage = entitylightning;
  ...
  protected boolean j(double d0, double d1, double d2)
  {
    ...
    List list = this.world.a(getBoundingBox());
  ...
  {
    return EnumDirection.fromType2(MathHelper.floor(this.yaw * 4.0F / 360.0F + 0.5D) & 0x3);
  }
