  ...
  implements Registers
{
    ...
    throws IOException
  {
    ...
    Seekable.File localFile = new Seekable.File(str5);
    ...
    {
      localFileWriter = str1 == null ? new OutputStreamWriter(System.out) : new FileWriter(str1);
  ...
  public Compiler(Seekable paramSeekable, String paramString)
    throws IOException
  {
  ...
  private void findBranchesInText(int paramInt1, DataInputStream paramDataInputStream, int paramInt2, Hashtable paramHashtable)
    throws IOException
  {
    int i = paramInt2 / 4;
    int j = paramInt1;
    ...
    for (int m = 0; m < i; j += 4)
    {
      ...
      case 0: 
        ...
        {
        case 9: 
          if (paramHashtable.put(new Integer(j + 8), Boolean.TRUE) == null) {
            k++;
          }
          break;
        case 12: 
          if (paramHashtable.put(new Integer(j + 4), Boolean.TRUE) == null) {
            k++;
          }
          break;
        }
      ...
      case 1: 
        ...
        case 0: 
        case 1: 
          if (paramHashtable.put(new Integer(j + i6 * 4 + 4), Boolean.TRUE) == null) {
            k++;
          }
          break;
        }
      ...
      case 2: 
        if (paramHashtable.put(new Integer(j & 0xF0000000 | i7 << 2), Boolean.TRUE) == null) {
          k++;
        }
        break;
      case 4: 
    ...
    if (this.printStats) {
      System.err.println("Found " + k + " additional possible branch targets in Text segment");
    }
  }
  
  private void findBranchesInData(DataInputStream paramDataInputStream, int paramInt1, Hashtable paramHashtable, int paramInt2, int paramInt3)
    throws IOException
  {
    int i = paramInt1 / 4;
    int j = 0;
    ...
    paramDataInputStream.close();
    if ((j > 0) && (this.printStats)) {
      System.err.println("Found " + j + " additional possible branch targets in Data segment");
    }
  }
  ...
  {
    String str = Long.toString(paramInt & 0xFFFFFFFF, 16);
    StringBuffer localStringBuffer = new StringBuffer("0x");
    for (int i = 8 - str.length(); i > 0; i--) {
  ...
  private class Option
  {
    ...
    {
      this.field = (paramString == null ? null : Compiler.class.getDeclaredField(paramString));
    }
  ...
  private static Integer parseInt(String paramString)
  {
    int i = 1;
    paramString = paramString.toLowerCase();
    if ((!paramString.startsWith("0x")) && (paramString.endsWith("m")))
    {
      paramString = paramString.substring(0, paramString.length() - 1);i = 1048576;
    }
    else if ((!paramString.startsWith("0x")) && (paramString.endsWith("k")))
    {
      paramString = paramString.substring(0, paramString.length() - 1);i = 1024;
    }
    int j;
    if ((paramString.length() > 2) && (paramString.startsWith("0x"))) {
      j = Integer.parseInt(paramString.substring(2), 16);
    } else {
