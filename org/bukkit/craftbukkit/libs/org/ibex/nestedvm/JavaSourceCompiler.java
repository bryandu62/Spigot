  ...
  extends Compiler
{
  ...
  {
    this.out.println(indents[this.indent] + paramString);
  }
  
  private void pblock(StringBuffer paramStringBuffer)
  {
    this.out.print(paramStringBuffer.toString());
  }
  ...
  {
    String str = "";
    for (int i = 0; i < indents.length; str = str + "    ")
    {
      indents[i] = str;i++;
    }
  }
  
  public JavaSourceCompiler(Seekable paramSeekable, String paramString, Writer paramWriter)
    throws IOException
  {
  ...
  private void endMethod(int paramInt)
  {
  ...
  private String constant(int paramInt)
  {
    if ((paramInt >= 4096) && (this.lessConstants))
    {
    ...
    throws Compiler.Exn, IOException
  {
    ...
    if (j == -1) {
      throw new Error("Actually read -1 at " + toHex(paramInt1));
    }
    for (int k = 0; k < i; paramInt1 += 4)
    {
      int m = j;
      j = k == i - 1 ? -1 : paramDataInputStream.readInt();
      if (paramInt1 >= this.endOfMethod)
      {
        endMethod();startMethod(paramInt1);
      }
  ...
  private void emitInstruction(int paramInt1, int paramInt2, int paramInt3)
    throws IOException, Compiler.Exn
  {
    if (paramInt2 == -1) {
    ...
    if ((this.runtimeStats) && (i != 0)) {
      p("inc(\"opcode: " + i + "\");");
    }
    switch (i)
    {
    case 0: 
      if ((this.runtimeStats) && (paramInt2 != 0)) {
        p("inc(\"opcode: 0/" + i4 + "\");");
      }
      switch (i4)
      {
    ...
    case 3: 
      ...
      int i9 = paramInt1 & 0xF0000000 | i5 << 2;
      emitInstruction(-1, paramInt3, -1);
      p("r31=" + constant(paramInt1 + 8) + ";");
      branch(paramInt1, i9);
      this.unreachable = true;
    ...
    case 17: 
      ...
      case 2: 
        if (i1 != 31) {
          throw new Compiler.Exn("FCR " + i1 + " unavailable");
        }
        p("r" + k + " = fcsr;");
        break;
      case 4: 
      ...
      case 6: 
        ...
        break;
      case 8: 
        int i10 = paramInt2 >>> 16 & 0x1;
        p("if(((fcsr&0x800000)!=0) == (" + i10 + "!=0)) {");
        this.indent += 1;
        emitInstruction(-1, paramInt3, -1);
        branch(paramInt1, paramInt1 + i8 * 4 + 4);
        this.indent -= 1;
      ...
      case 16: 
        ...
        case 62: 
          ...
          break;
        default: 
          throw new Compiler.Exn("Invalid Instruction 17/" + j + "/" + i4);
        }
        break;
      case 17: 
        switch (i4)
  ...
  private void memWrite(String paramString1, String paramString2)
  {
    if (this.nullPointerCheck) {
      p("nullPointerCheck(" + paramString1 + ");");
    }
    if (this.onePage) {
  ...
  private static String getFloat(int paramInt)
  {
    return "(Float.intBitsToFloat(f" + paramInt + "))";
  }
  
  private static String getDouble(int paramInt)
  {
    return "(Double.longBitsToDouble(((f" + (paramInt + 1) + "&0xffffffffL) << 32) | (f" + paramInt + "&0xffffffffL)))";
  }
  
  private static String setFloat(int paramInt, String paramString)
  {
    return "f" + paramInt + "=Float.floatToRawIntBits(" + paramString + ");";
  }
  
  private static String setDouble(int paramInt, String paramString)
  {
    return "{ long l = Double.doubleToLongBits(" + paramString + "); " + "f" + (paramInt + 1) + " = (int)(l >>> 32); f" + paramInt + " = (int)l; }";
  }
}
