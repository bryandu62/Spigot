  ...
  implements RolloverStrategy
{
    ...
    throws SecurityException
  {
    ...
    {
      int fileIndex;
      if ((fileIndex = purge(this.minIndex, this.maxIndex, manager)) < 0) {
        return null;
      }
  ...
  private int purgeDescending(int lowIndex, int highIndex, RollingFileManager manager)
  {
    ...
    StringBuilder buf = new StringBuilder();
    manager.getPatternProcessor().formatFileName(buf, Integer.valueOf(lowIndex));
    ...
    for (int i = lowIndex; i <= highIndex; i++)
    {
      ...
      buf.setLength(0);
      manager.getPatternProcessor().formatFileName(buf, Integer.valueOf(i + 1));
  ...
  private int purgeAscending(int lowIndex, int highIndex, RollingFileManager manager)
  {
    ...
    List<FileRenameAction> renames = new ArrayList();
    StringBuilder buf = new StringBuilder();
    manager.getPatternProcessor().formatFileName(buf, Integer.valueOf(highIndex));
